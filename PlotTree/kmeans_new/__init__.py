...
def kmeans(
    ...
    if distance == 'euclidean':
        ...
        pairwise_distance_function = partial(pairwise_plot, device=device)
    else:
...
def pairwise_plot(data1, data2, device=torch.device('cpu')):
    data1, data2 = data1.to(device), data2.to(device)

    A, A_ids = data1[:,:-1], data1[:,-1]
    B, B_ids = data2[:,:-1], data2[:,-1]
    # N*1*M
    A = A.unsqueeze(dim=1)
    # 1*N*M
    B = B.unsqueeze(dim=0)
    ...
    cosine = A_normalized * B_normalized
    print("Using both temporal and semantic distance in a formulation !!!")
    distance_ids = torch.abs(A_ids.unsqueeze(1) - B_ids.unsqueeze(0))   # [N, M]
